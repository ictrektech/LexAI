...
GARBLED_TEXT_CJK_PUNCT_RATIO = _env_float(
    "DOCREADER_PDF_GARBLED_TEXT_CJK_PUNCT_RATIO", 0.30
)
    ...
    for ch in compact:
        ...
        if ch.isalnum():
            if 0x80 <= code <= 0x00FF:
                bad += 1
            continue
        if cat[0] in {"C", "S"} or 0x80 <= code <= 0x00FF:
            bad += 1
        elif cat[0] == "P":
            punct += 1
        ...
        try:
            ...
            for i in range(page_count):
                ...
                try:
                    ...
                    cls = "scanned" if garbled_text else _classify_page(
                        ratio, len(plain.strip())
                    )
                    if garbled_text:
                        logger.info(
                            "PDFParser: page %d of %s has garbled text layer; "
                            "routing to scanned/OCR path",
                            i + 1,
                            self.file_name,
                        )

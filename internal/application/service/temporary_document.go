...
func temporaryDocumentImageOCRMaxPages() int {
	return temporaryDocumentEnvPositiveInt("WEKNORA_CHAT_ATTACHMENT_OCR_MAX_PAGES", defaultTemporaryDocumentImageOCRMaxPages)
}
...
func temporaryDocumentOCRConcurrency() int {
	return temporaryDocumentEnvPositiveInt("WEKNORA_CHAT_ATTACHMENT_OCR_CONCURRENCY", defaultTemporaryDocumentOCRConcurrency)
}

// temporaryDocumentEnvPositiveInt reads a positive integer from the environment, falling back to
// def when the variable is unset, non-numeric, or non-positive.
func temporaryDocumentEnvPositiveInt(key string, def int) int {

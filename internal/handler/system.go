...
func (h *SystemHandler) RunDeployUpdate(c *gin.Context) {
	...
	start := time.Now()
	script := fmt.Sprintf(
		`cd /lexai-deploy && { echo "[INFO] deploy update requested at $(date -Is)"; if mkdir .update-and-deploy.lock 2>/dev/null; then trap 'rmdir .update-and-deploy.lock' EXIT; ./update-and-deploy.sh --platform %s; else echo "[INFO] deploy update already running"; fi; } >> update-and-deploy.log 2>&1`,
		platform,
	)

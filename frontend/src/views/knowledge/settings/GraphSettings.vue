...
const normalizedTemplate = (raw: any): GraphExtractConfig => ({
  ...
  nodes: Array.isArray(raw?.nodes) ? raw.nodes.map((node: any) => ({
    ...
    attributes: Array.isArray(node?.attributes) ? node.attributes.filter((attr: unknown) => typeof attr === 'string') : []
  })) : [],
  relations: Array.isArray(raw?.relations) ? raw.relations.map((relation: any) => ({
...
const downloadTemplate = () => {
  const blob = new Blob([JSON.stringify(localGraphExtract.value, null, 2)], { type: 'application/json' })
...
const uploadTemplate = async (event: Event) => {
  ...
  if (!file) return
  try {
    localGraphExtract.value = normalizedTemplate(JSON.parse(await file.text()))
    handleNodesChange()

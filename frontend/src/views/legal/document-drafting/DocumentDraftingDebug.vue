    ...
    <main class="debug-content">
      ...
      <template v-else>
        ...
        <section class="panel split-panel">
          <header><div><span>02</span><h2>Inspect</h2></div><small data-testid="debug-inspect-summary">{{ inspectSummary }}</small></header>
          <div class="split-content">
            ...
            <div class="blob-area">
              <div v-if="inspectBlobs.length" class="blob-tabs" role="tablist">
                <button v-for="blob in inspectBlobs" :key="blob.id" type="button" class="blob-tab" :class="{ active: selectedInspectBlob?.id === blob.id }" :data-testid="`debug-inspect-${blob.kind}`" role="tab" :aria-selected="selectedInspectBlob?.id === blob.id" @click="selectInspectBlob(blob)">
                  <strong>{{ blobLabel(blob.kind) }}</strong><small>{{ formatBytes(blob.size) }}</small>
                </button>
              </div>
              <div v-if="selectedInspectBlob" class="blob-viewer" data-testid="debug-inspect-viewer">
                <div class="blob-viewer__heading"><strong>{{ blobLabel(selectedInspectBlob.kind) }}</strong><span>{{ selectedInspectBlob.kind }} · {{ formatBytes(selectedInspectBlob.size) }}</span></div>
                <p v-if="isBlobLoading(selectedInspectBlob.id)" class="empty">{{ t('documentDrafting.debug.loading') }}</p>
                <pre v-else-if="hasBlobContent(selectedInspectBlob.id)">{{ inspectBlobContent(selectedInspectBlob) }}</pre>
                <p v-else class="empty">{{ t('documentDrafting.debug.expandToLoad') }}</p>
              </div>
              <p v-else class="empty">{{ t('documentDrafting.debug.expandToLoad') }}</p>
            </div>
          </div>
        ...
        <section class="panel">
          ...
          <div v-if="plannerBlobs.length" class="blob-tabs blob-buttons">
            <button v-for="blob in plannerBlobs" :key="blob.id" type="button" class="blob-tab" :class="{ active: selectedPlannerBlob?.id === blob.id }" :data-testid="`debug-planner-${blob.kind}`" role="tab" :aria-selected="selectedPlannerBlob?.id === blob.id" @click="selectPlannerBlob(blob)">
              <strong>{{ blobLabel(blob.kind) }}</strong><small>{{ formatBytes(blob.size) }}</small>
            </button>
          </div>
          <div v-if="selectedPlannerBlob" class="blob-viewer" data-testid="debug-planner-viewer">
            <div class="blob-viewer__heading"><strong>{{ blobLabel(selectedPlannerBlob.kind) }}</strong><span>{{ selectedPlannerBlob.kind }} · {{ formatBytes(selectedPlannerBlob.size) }}</span></div>
            <p v-if="isBlobLoading(selectedPlannerBlob.id)" class="empty">{{ t('documentDrafting.debug.loading') }}</p>
            <pre v-else-if="hasBlobContent(selectedPlannerBlob.id)">{{ prettyBlob(selectedPlannerBlob) }}</pre>
            <p v-else class="empty">{{ t('documentDrafting.debug.expandToLoad') }}</p>
          </div>
        </section>
        ...
        <section class="panel">
          ...
          <button type="button" class="secondary plan-json-toggle" data-testid="debug-plan-json-toggle" @click="planJsonVisible = !planJsonVisible">{{ t('documentDrafting.debug.planJson') }}{{ planJsonVisible ? ' ↑' : ' ↓' }}</button>
          <pre v-if="planJsonVisible" data-testid="debug-plan-json">{{ JSON.stringify(job.plan || {}, null, 2) }}</pre>
        </section>
...
const blobLoading = reactive<Record<string, boolean>>({})
const comparisonDebug = reactive<Record<string, DocumentEditDebug>>({})
const diffLoading = ref(false)
const selectedInspectBlobId = ref('')
const selectedPlannerBlobId = ref('')
const planJsonVisible = ref(false)
...
const latestInspectStage = computed(() => latestInspect.value || [...(debug.value?.stages || [])].reverse().find(stage => stage.stage === 'Inspect'))
const planInput = computed(() => planStage.value?.input_summary || {})
const planOutput = computed(() => planStage.value?.output_summary || {})
const plannerBlobs = computed(() => blobsForLatestStage('planner_', planStage.value?.id))
const planOperations = computed<any[]>(() => Array.isArray((job.value?.plan as any)?.operations) ? (job.value?.plan as any).operations : [])
const sourceTextBlob = computed(() => inspectBlobs.value.find(blob => blob.kind === 'inspect_text' || blob.kind === 'inspect_plain'))
const cleanTextBlob = computed(() => (debug.value?.blobs || []).find(blob => blob.kind === 'clean_text'))
const inspectBlobs = computed(() => {
  const byKind = new Map(blobsForLatestStage('', latestInspectStage.value?.id, ['inspect_text', 'inspect_plain', 'inspect_annotated', 'semantic_snapshot']).map(blob => [blob.kind, blob]))
  return [byKind.get('inspect_plain') || byKind.get('inspect_text'), byKind.get('inspect_annotated'), byKind.get('semantic_snapshot')].filter(Boolean) as DocumentEditDebugBlob[]
})
const selectedInspectBlob = computed(() => inspectBlobs.value.find(blob => blob.id === selectedInspectBlobId.value) || null)
const selectedPlannerBlob = computed(() => plannerBlobs.value.find(blob => blob.id === selectedPlannerBlobId.value) || null)
const inspectSummary = computed(() => latestInspect.value ? `${latestInspect.value.engine_name || '—'} · ${latestInspect.value.duration_ms}ms · ${t('documentDrafting.debug.attempt', { count: latestInspect.value.attempt })}` : t('documentDrafting.debug.notRecorded'))
...
function blobsForLatestStage(prefix: string, stageId?: string, allowedKinds?: string[]) {
  const blobs = (debug.value?.blobs || []).filter(blob => (prefix ? blob.kind.startsWith(prefix) : true) && (!allowedKinds || allowedKinds.includes(blob.kind)))
  if (stageId) return blobs.filter(blob => blob.stage_run_id === stageId)
  const latestByKind = new Map<string, DocumentEditDebugBlob>()
  blobs.forEach(blob => latestByKind.set(blob.kind, blob))
  return [...latestByKind.values()]
}
function hasBlobContent(id: string) { return Object.prototype.hasOwnProperty.call(blobContents, id) }
function isBlobLoading(id: string) { return Boolean(blobLoading[id]) }
function blobLabel(kind: string) {
  switch (kind) {
    case 'inspect_text':
    case 'inspect_plain': return 'Plain Text'
    case 'inspect_annotated': return 'Annotated'
    case 'semantic_snapshot': return 'Semantic Snapshot'
    case 'planner_messages': return 'Planner Messages'
    case 'planner_response_initial': return 'Planner Response'
    case 'planner_response_repair': return 'Planner Repair'
    default: return kind
  }
}
function inspectBlobContent(blob: DocumentEditDebugBlob) {
  const value = blobContents[blob.id] || ''
  return blob.kind === 'semantic_snapshot' ? prettyContent(value) : value
}
...
function prettyContent(value: string) { try { return JSON.stringify(JSON.parse(value), null, 2) } catch { return value } }
...
async function loadBlob(blob: DocumentEditDebugBlob) {
  if (hasBlobContent(blob.id) || blobLoading[blob.id]) return
  blobLoading[blob.id] = true
  try {
    blobContents[blob.id] = await getDocumentEditDebugBlob(blob.job_id, blob.stage_run_id, blob.kind)
  } catch (cause: any) {
    MessagePlugin.error(cause?.message || t('documentDrafting.debug.blobLoadFailed'))
  } finally {
    delete blobLoading[blob.id]
  }
}
async function selectInspectBlob(blob: DocumentEditDebugBlob) {
  selectedInspectBlobId.value = blob.id
  await loadBlob(blob)
}
async function selectPlannerBlob(blob: DocumentEditDebugBlob) {
  selectedPlannerBlobId.value = blob.id
  await loadBlob(blob)
}
...
watch(inspectBlobs, (blobs) => {
  if (!blobs.some(blob => blob.id === selectedInspectBlobId.value)) selectedInspectBlobId.value = ''
})
watch(plannerBlobs, (blobs) => {
  if (!blobs.some(blob => blob.id === selectedPlannerBlobId.value)) selectedPlannerBlobId.value = ''
})
watch(() => route.params.jobId, () => {
  Object.keys(blobContents).forEach(key => delete blobContents[key])
  Object.keys(blobLoading).forEach(key => delete blobLoading[key])
  selectedInspectBlobId.value = ''
  selectedPlannerBlobId.value = ''
  planJsonVisible.value = false
  void load()
})
onMounted(() => { void load() })
...
.blob-tabs { display:flex; flex-wrap:wrap; gap:8px; } .blob-tab { min-width:145px; padding:10px 12px; display:inline-flex; flex-direction:column; align-items:flex-start; gap:4px; border:1px solid var(--legal-border); border-radius:6px; color:var(--legal-text-primary); background:var(--legal-bg-surface); cursor:pointer; text-align:left; } .blob-tab strong { font-size:11px; } .blob-tab small { color:var(--legal-text-secondary); font-size:10px; } .blob-tab.active { border-color:var(--legal-brand); box-shadow:0 0 0 1px var(--legal-brand); background:var(--legal-ai-soft); } .blob-viewer { margin-top:14px; } .blob-viewer__heading { margin-bottom:8px; display:flex; align-items:baseline; justify-content:space-between; gap:10px; } .blob-viewer__heading strong { font-size:12px; } .blob-viewer__heading span { color:var(--legal-text-secondary); font:10px ui-monospace, monospace; } .blob-viewer pre { margin:0; }
.parameter-grid { display:grid; grid-template-columns:repeat(6,1fr); gap:8px; } .parameter-grid div { padding:10px; border:1px solid var(--legal-border); border-radius:5px; } .parameter-grid span,.parameter-grid strong { display:block; font-size:9px; } .parameter-grid span { color:var(--legal-text-secondary); } .parameter-grid strong { margin-top:5px; } .blob-buttons { display:flex; flex-wrap:wrap; gap:7px; } .plan-json-toggle { margin-top:0!important; } details { border-top:1px solid var(--legal-border); } details summary { padding:11px 0; cursor:pointer; font-size:10px; }

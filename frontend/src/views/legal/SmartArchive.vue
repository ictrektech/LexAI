  ...
  <section class="archive-page">
    <header class="archive-header">
      ...
      <div class="header-actions">
        <button class="notification-button" type="button" @click="tab = 'reminders'; void loadReminders()"><t-icon name="notification" /><span v-if="unreadCount" class="notification-count">{{ unreadCount }}</span></button>
        <button class="primary-button" type="button" :disabled="uploading" @click="fileInput?.click()"><t-icon name="upload" /> {{ t('smartArchive.importFiles') }}</button>
        <input ref="fileInput" type="file" hidden multiple accept=".pdf,.doc,.docx,.xls,.xlsx,.jpg,.jpeg,.png,.webp,application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document,application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,image/jpeg,image/jpg,image/png,image/webp" @change="onFileInput" />
      </div>
    </header>

    <div class="archive-toolbar">
      <nav class="archive-tabs">
        <button v-for="item in tabs" :key="item.id" type="button" :class="{ active: tab === item.id }" @click="selectTab(item.id)"><t-icon :name="item.icon" /> {{ item.label }}</button>
      </nav>
      <div class="toolbar-filters"><select v-model="documentTypeFilter" @change="runSearch"><option value="">{{ t('smartArchive.allTypes') }}</option><option v-for="type in documentTypes" :key="type" :value="type">{{ documentTypeLabel(type) }}</option></select><div class="search-box"><t-icon name="search" /><input v-model="query" :placeholder="t('smartArchive.searchPlaceholder')" @keydown.enter="runSearch" /><button v-if="query" type="button" @click="query = ''; runSearch()"><t-icon name="close" /></button></div></div>
    </div>
...
import type { ArchiveDocument, ArchiveExtractionStatus, ArchiveReminder, ArchiveReminderCandidate, ArchiveReminderStatus } from '@/api/smart-archive'
...
async function runBulkAction(action: 'archive' | 'restore' | 'delete') {
...
const loadReminders = () => Promise.all([store.loadReminders(), store.loadReminderCandidates()]).catch((e: any) => MessagePlugin.error(e?.message || t('smartArchive.loadFailed')))
async function selectTab(value: string) { selectedIds.value = []; reminderSelectedIds.value = []; candidateSelectedIds.value = []; tab.value = value; if (value === 'documents' || value === 'queue') await loadDocuments(); if (value === 'reminders') { await store.loadReminders(); await store.loadReminderCandidates(); await store.loadNotifications() } }
...
async function markRead(id: string) { await import('@/api/smart-archive').then(({ markArchiveNotificationRead }) => markArchiveNotificationRead(id)); await store.loadNotifications() }
...
onMounted(async () => { await store.loadSettings(); await loadMembers(); await loadDocuments(); await store.loadReminders(); await store.loadReminderCandidates(); syncStatusPolling(hasProcessingDocuments.value) }); onBeforeUnmount(() => { stopStatusPolling(); store.disconnect() })
</script>

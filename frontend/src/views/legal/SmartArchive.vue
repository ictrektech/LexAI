  ...
  <section class="archive-page">
    ...
    <main class="archive-content">
      ...
      <template v-else-if="tab === 'reminders'">
        ...
        <div v-if="store.notifications.length" class="notification-list"><h3>{{ t('smartArchive.notifications') }}</h3><article v-for="notification in store.notifications" :key="notification.id" :class="{ unread: !notification.read_at }"><div><b>{{ notification.title }}</b><p>{{ notification.body }}</p><small>{{ formatDate(notification.created_at) }}</small></div><div class="notification-actions"><button v-if="!notification.read_at" class="secondary-button" type="button" @click="markRead(notification.id)">{{ t('smartArchive.markRead') }}</button><button class="secondary-button secondary-button--danger" type="button" @click="deleteNotification(notification)">{{ t('smartArchive.deleteNotification') }}</button></div></article></div>
      </template>

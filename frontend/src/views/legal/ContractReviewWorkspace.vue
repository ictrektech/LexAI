<template>
  <section class="review-list">
    ...
    <div v-if="selectedCount" class="review-list__bulk-bar">
      ...
      <div>
        <button type="button" class="bulk-action" :disabled="bulkWorking || (!archived && !archivableSelectedCount)" @click="runBulk(archived ? 'restore' : 'archive')"><t-icon :name="archived ? 'rollback' : 'archive'" /><span>{{ archived ? t('contractReview.bulkRestore') : t('contractReview.bulkArchive') }}</span></button>
        <button type="button" class="bulk-action danger" :disabled="bulkWorking" @click="runBulk('delete')"><t-icon name="delete" /><span>{{ t('contractReview.bulkDelete') }}</span></button>
      </div>
    </div>
    <div class="review-list__table">
      <div class="review-list__table-head"><span class="review-check"><input type="checkbox" :checked="allSelected" :indeterminate.prop="someSelected" :aria-label="t('contractReview.selectAll')" @change="toggleAll" /></span><span>{{ t('contractReview.document') }}</span><span>{{ t('contractReview.statusLabel') }}</span><span>{{ t('contractReview.party') }}</span><span>{{ t('contractReview.updated') }}</span><span /></div>

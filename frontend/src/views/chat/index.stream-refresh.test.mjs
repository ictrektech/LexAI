...
test('answer stream chunks merge snapshots without duplicating the same answer', () => {
  ...
  assert.match(handlerSource, /answerEvent\.content = mergeStreamText\(answerEvent\.content \|\| previousAnswer,\s*data\.content\)/)
...
test('completed agent answers leave streaming text and render markdown immediately', () => {
  ...
  assert.match(agentStreamSource, /<div v-if="isAnswerEventStreaming\(event\)" v-html="renderAnswerContent\(typedAnswer\)" \/>/)
...
test('switching back to an in-flight stream anchors at the active turn start', () => {
  ...
  assert.match(source, /let inFlightTurnHistoryLoadGuardUntil = 0/)
  ...
  assert.match(source, /const findActiveTurnAnchorRenderedIndex = \(\) => \{[\s\S]*const activeTurnUserIndex = findActiveTurnUserRenderedIndex\(\);[\s\S]*return activeTurnUserIndex >= 0 \? activeTurnUserIndex : findActiveAssistantRenderedIndex\(\);[\s\S]*\}/)
  ...
  assert.match(source, /const finishRestoredInFlightTurnAnchor = \(targetSessionId = session_id\.value\) => \{[\s\S]*clearInFlightTurnScrollPreservation\(targetSessionId\)/)
  assert.match(source, /if \(anchorIndex >= 0 && applyRenderedMessageAnchor\(anchorIndex\)\) \{[\s\S]*finishRestoredInFlightTurnAnchor\(targetSessionId\);[\s\S]*return;/)
  assert.match(source, /if \(Date\.now\(\) > inFlightTurnAnchorUntil\) \{[\s\S]*clearInFlightTurnAnchor\(\);[\s\S]*clearInFlightTurnScrollPreservation\(targetSessionId\)/)
  ...
  assert.match(source, /if \(preserveInFlightTurnScrollSessionId\.value !== targetSessionId\) return;[\s\S]*if \(hasActiveStream\(targetSessionId\) \|\| findActiveIncompleteAssistant\(targetSessionId\)\) \{[\s\S]*userHasScrolledUp\.value = true;/)
  assert.match(source, /const finishInFlightTurnAnchor = \(targetSessionId = session_id\.value\) => \{/)
  assert.match(source, /const finishInFlightTurnAnchor = \(targetSessionId = session_id\.value\) => \{[\s\S]*clearInFlightTurnScrollPreservation\(targetSessionId\)/)
  assert.match(source, /const hasTargetInFlightTurn = hasActiveStream\(targetSessionId\) \|\|[\s\S]*inFlightTurnCache\.has\(targetSessionId\) \|\|[\s\S]*inFlightAnswerSnapshot\.has\(String\(targetSessionId\)\)/)
  ...
  assert.match(source, /Date\.now\(\) < inFlightTurnHistoryLoadGuardUntil/)
...
test('in-flight turn cache merges snapshots instead of overwriting with stream tails', () => {
  ...
  assert.match(source, /const isSameInFlightAssistantMessage = \(left,\s*right,\s*targetSessionId = session_id\.value\) => \{/)
  assert.match(source, /leftText\.includes\(rightText\) \|\| rightText\.includes\(leftText\)/)
  ...
  assert.match(source, /isSameInFlightAssistantMessage\(message,\s*incoming,\s*targetSessionId\)/)
  ...
  assert.match(source, /const restoredAssistants = restored\.filter\(\(message\) => message\.role === 'assistant'\)/)
  assert.match(source, /isSameInFlightAssistantMessage\(message,\s*restoredMessage,\s*targetSessionId\)/)
  assert.match(source, /isSameInFlightAssistantMessage\(message,\s*assistant,\s*targetSessionId\)/)
  assert.match(source, /const existing = messagesList\.find\(\(message\) =>\s*messageIdentityMatches\(message,\s*cachedMessage\) \|\|\s*isSameInFlightAssistantMessage\(message,\s*cachedMessage,\s*targetSessionId\)/)
})

test('streaming answer text is cached per session while switching conversations', () => {
  assert.match(source, /const inFlightFullContentBySession = new Map\(\)/)
  assert.match(source, /inFlightFullContentBySession\.set\(sessionKey,\s*value\)/)
  assert.match(source, /const cached = String\(inFlightFullContentBySession\.get\(sessionKey\) \|\| ''\)/)
  assert.match(source, /const syncLatestInFlightAnswerIntoMessage = \(targetSessionId,\s*message\) => \{[\s\S]*inFlightAnswerSnapshot\.get\(String\(targetSessionId \|\| ''\)\)[\s\S]*getSessionFullContent\(targetSessionId\)[\s\S]*lastAnswerEvent\.content = mergeStreamText\(lastAnswerEvent\.content,\s*latestAnswer\)/)
  assert.match(source, /const restoreInFlightFullContent = \(targetSessionId = session_id\.value\) => \{[\s\S]*applyAnswerSnapshotToMessage\(targetSessionId,\s*targetMessage\);[\s\S]*refreshMessageRow\(targetMessage\);/)
  assert.match(source, /const restoredAnswer = restoreInFlightFullContent\(targetSessionId\);[\s\S]*if \(!chunks\?\.length\) \{[\s\S]*cacheCurrentInFlightTurn\(targetSessionId\);[\s\S]*anchorRestoredInFlightTurn\(targetSessionId\);[\s\S]*return;/)
  assert.match(source, /syncLatestInFlightAnswerIntoMessages\(targetSessionId,\s*tail\);[\s\S]*tail\.forEach\(\(message\) => rememberMessageAnswerSnapshot\(targetSessionId,\s*message\)\);[\s\S]*mergeInFlightTurnCache\(targetSessionId,\s*tail\)/)
  assert.match(source, /setSessionFullContent\(chunkSessionId,\s*mergeStreamText\(getSessionFullContent\(chunkSessionId\),\s*data\.content\)\)/)
  assert.doesNotMatch(source, /setSessionFullContent\(chunkSessionId,\s*getSessionFullContent\(chunkSessionId\)\)/)
  assert.match(source, /clearData = \(abortStreams = true\) => \{[\s\S]*isReplying\.value = false;[\s\S]*\/\/ Stop any IM-reply recovery poll/)
  assert.doesNotMatch(source, /clearData = \(abortStreams = true\) => \{[\s\S]*clearSessionFullContent\(session_id\.value\)[\s\S]*\/\/ Stop any IM-reply recovery poll/)
  assert.match(source, /onTurnComplete: \(message\) => \{[\s\S]*clearSessionFullContent\(completedSessionId\)/)
})

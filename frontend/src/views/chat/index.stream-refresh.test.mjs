...
test('switching back to an in-flight stream anchors at the active turn start', () => {
  ...
  assert.match(source, /activeInFlightTurnAnchorSessionId\.value = targetSessionId[\s\S]*inFlightTurnAnchorUntil = Date\.now\(\) \+ 1500/)
  ...
  assert.match(source, /pendingInFlightTurnAnchorSessionId\.value =[\s\S]*hasActiveStream\(targetSessionId\) \|\| inFlightTurnCache\.has\(targetSessionId\) \? targetSessionId : ''/)

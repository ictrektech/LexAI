...
# Build and push the ictrek LexAI service images.
#
...
Builds the LexAI service images and records each service tag in Feishu.
...
if data.get("code") != 0:
    raise SystemExit(f"append component column failed: {data}")
PY
}
...
find_or_create_component_column() {
  ...
  local repo_uri="$4"
  local column_count end_col resp_file found_col

  column_count="$(get_sheet_column_count "$token" "$sheet_id")"
  end_col="$(column_letter "$column_count")"
  resp_file="$(mktemp)"
  get_range_values "$token" "${sheet_id}!A1:${end_col}2" > "$resp_file"

  found_col="$(python3 - "$component_name" "$resp_file" <<'PY'
import json, sys
target = sys.argv[1]
...
PY
  ...
  append_component_column "$token" "$sheet_id"
  echo $((column_count + 1))
}

...
合同审查可以用「快速问答」或「智能推理」两种方式。单份合同审查、条款定位、风险点识别通常先用快速问答；需要工具调用、复杂步骤规划或合同审查 Skill 时，再使用智能推理。
...
1. 进入「智能体」，创建合同审查智能体。
2. 在提示词模板中选择「合同审查」模板。
3. 指定用于合同审查的知识库，例如合同模板库、法律条文库或项目合同库。
4. 保存智能体后，选择该智能体开始会话。
5. 在输入框里上传或选择要审查的合同，并说明审查立场和输出格式。
...
5. 在工具或 Skill 配置中选择合同审查 Skill。
6. 保存后，选择该合同审查智能体开始会话。

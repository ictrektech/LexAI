...
`WEKNORA_CHAT_RESERVED_CONCURRENCY` 是给在线聊天保留的最低并发，不让后台 LLM 任务占用。后台 LLM 可用槽位近似为：
...
`BGE_VLLM_MAX_NUM_SEQS` 是 bge-m3 vLLM 服务的序列并发。想给聊天检索保留余量时，应让它高于 `CONCURRENCY_POOL_SIZE`。
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | --- |
| Thor `192.168.1.81` | 6 | 2 | 2 | 2 / 2 | 2 | 2 | QA/Graph/Wiki/Question 共用 9B vLLM，后台 LLM 最多占 4 个槽位。 |
| tc232 9B vLLM | 6 | 2 | 2 | 2 / 2 | 2 | 2 | 和 Thor 保持同样的 LLM 预算形状。 |
...
这样文档 embedding 最多使用 9 个请求，同时给聊天检索留出约 3 个 bge-m3 服务槽位。

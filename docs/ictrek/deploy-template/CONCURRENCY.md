...
| `VLLM_MAX_NUM_SEQS` / `WEKNORA_MAIN_QA_MODEL_CONCURRENCY` | `12` | QA vLLM 最多可接收的序列数、LexAI 主模型请求入口 | 满上下文下实际可长期并行的请求数 | `12` 是 tc97 当前服务请求上限；应用侧主模型入口必须与它一致。 |
...
| `WEKNORA_MODEL_MAX_CONCURRENCY` | `6` | **全部后台任务同时进入主 QA 模型的总数**：Graph、Wiki、摘要、自动问题、需要主 QA 的 VLM | 在线聊天、bge-m3 embedding、文字解析/分块/向量化 worker | 让后台任务最多使用 12 个主模型请求槽中的 6 个，另外 6 个保留给聊天。 |
| `WEKNORA_GRAPH_LLM_CONCURRENCY` | `2` | 单个文档内 Graph chunk 的 LLM 并发 | 其他类型任务、在线聊天 | 防止一个 Graph 任务自己拆出过多并行 LLM 请求；仍受上面的全局 `MODEL_MAX=6` 约束。 |
...
| `WEKNORA_WIKI_ASYNQ_CONCURRENCY` | `4` | Wiki worker 数 | 主 QA 模型请求数 | Wiki 排队、准备和落库可并行；真正调用主 QA 时仍受 `MODEL_MAX=6` 限制。 |
...
因此，`WEKNORA_MODEL_MAX_CONCURRENCY=6` 的目的不是降低聊天并发，而是把后台对 **同一个 QA vLLM** 的长请求限制在非聊天半区。聊天请求不经过这个后台闸门；文字解析、分块和 embedding 也不经过它。它是应用侧保护，仍要用 vLLM `waiting` 指标回验；如果聊天时持续出现 `waiting > 0`，应继续降低后台模型负载、Graph/Wiki/VLM 并发或上下文，而不是提高 worker 数。
...
```text
Maximum concurrency for 18,432 tokens per request: 141.00x
```

这个数表示满长请求下的有效 KV 并发。`VLLM_MAX_NUM_SEQS` 和 `WEKNORA_MAIN_QA_MODEL_CONCURRENCY` 不能超过服务实际配置；如果启动日志里的有效 KV 并发低于服务请求上限，后台主模型并发必须按这个实测值扣除聊天预留。tc97 当前有效 KV 并发高于 `VLLM_MAX_NUM_SEQS=12`，所以按请求上限分配：12 个主模型入口、6 个聊天保留、6 个后台主模型槽。
...
`WEKNORA_MODEL_MAX_CONCURRENCY` 是**后台主模型的硬闸门**，不是 `VLLM_MAX_NUM_SEQS` 的别名。它应先按 `min(VLLM_MAX_NUM_SEQS, floor(<n>)) - WEKNORA_CHAT_RESERVED_CONCURRENCY` 估算，再以聊天期间的 vLLM `waiting` 指标回验。tc97 在 `18432` 上下文、qwen `gpu_memory_utilization=0.65`、`VLLM_MAX_NUM_SEQS=12` 下实测满长 KV 容量高于请求上限，当前按 12 个服务请求槽分配：6 个聊天保留，后台主模型最多 6 个。
...
如果两个值都大于 0，且 `main <= reserved`，LexAI 仍会保留 1 个后台槽位，避免 Graph/Wiki/Question 完全不执行。如果任意一个值为空或为 `0`，后台 LLM 限流不会启用。tc97 的最终后台上限仍以 `WEKNORA_MODEL_MAX_CONCURRENCY=6` 为准。
...
`WEKNORA_ASYNQ_*_CONCURRENCY` 不等同于主 QA 模型并发。tc97 Thor 的后台池为 `core=4 / postprocess=2 / enrichment=2 / maintenance=1 / shared=0`，让文字解析和 embedding 继续推进，同时由 `WEKNORA_MODEL_MAX_CONCURRENCY=6` 统一限制 Graph/VLM/Summary/Question/Wiki 等后台主模型请求。
...
不要只用 `VLLM_MAX_NUM_SEQS - WEKNORA_CHAT_RESERVED_CONCURRENCY` 计算后台模型并发。vLLM 在启动日志里会输出当前上下文下的有效 KV 并发，例如 tc97 Thor 在 `VLLM_MAX_MODEL_LEN=18432`、`VLLM_MAX_NUM_SEQS=12`、`gpu_memory_utilization=0.65` 下实测：

```text
Maximum concurrency for 18,432 tokens per request: 141.00x
```

这说明在当前显存占用率下，tc97 的满长 KV 容量不是瓶颈；瓶颈是 `VLLM_MAX_NUM_SEQS=12` 这个请求上限。为了给聊天保留 6 个可用槽位，tc97 的后台主模型并发设为 `6`。如果另一台 Thor 或另一套模型的启动日志低于 `VLLM_MAX_NUM_SEQS`，必须按较小值重新计算，不能照抄 tc97。
...
| --- | ---: | ---: | --- | ---: | ---: | ---: | --- |
| Thor M114/T5000 `192.168.1.97` | 12 | 6 | core=4 / postprocess=2 / enrichment=2 / maintenance=1 / shared=0 / wiki=4 | 6 | 2 | 4 / 4 | QA/Graph/Wiki/Question 共用 9B vLLM，`VLLM_MAX_MODEL_LEN=18432`，qwen `gpu_memory_utilization=0.65`，bge `gpu_memory_utilization=0.1`。当前实测满长 KV 容量高于请求上限，所以后台主模型并发按 `12 - 6 = 6` 配。 |
...
| 现象 | 优先调整 |
| --- | --- |
| 文档入库时聊天变慢 | 先看 vLLM 启动日志里的 `Maximum concurrency for ... tokens per request`。后台主模型并发应小于等于 `min(VLLM_MAX_NUM_SEQS, 实测满长并发) - 聊天预留`。tc97 当前是 `min(12, 141) - 6 = 6`；不要只看 `VLLM_MAX_NUM_SEQS`，也不要只看显存。 |
...
tc97 当前配置下，后台主模型调用正常应长期压在 6 个以内；用户聊天同时发生时，qwen running 可以短时超过 6，但不应持续出现 `waiting > 0`。bge-m3 当前是 16 槽，文档 embedding 应用侧最多 8 个请求；如果 bge `waiting > 0`，先降 `CONCURRENCY_POOL_SIZE` 或后台 worker。
...
2. tc97 Thor 保持 `VLLM_MAX_NUM_SEQS=12`、聊天预留 `6`，后台主模型调用固定为 `WEKNORA_MODEL_MAX_CONCURRENCY=6`。这不是把系统总并发降到 6，而是阻止 Graph/Wiki/摘要/VLM 占用预留给聊天的 6 个 QA vLLM 请求槽。

...
本文说明 ictrek 部署模板里的并发配置。实际部署时，把这些值写到目标机 env 文件：`.env`、`.env.tc232`、`.env.thor` 或机器专用 env。本文是通用参考，Thor/tc97 章节只给当前实测示例；tc232、AMD 机器和新主机也要使用同一套参数含义，并按本机模型、上下文、vLLM 容量和响应延迟重算取值。

...
| `WEKNORA_ASYNQ_CORE_CONCURRENCY` / `POSTPROCESS` / `ENRICHMENT` / `MAINTENANCE` / `SHARED` | `2 / 1 / 1 / 1 / 0` | 各后台 worker 池的进程内并发 | 主 QA 模型请求数 | core 保证文字解析；postprocess 保证解析后的轻量扇出；enrichment 只有 1 个 worker；Thor 关闭 shared，避免弹性池绕过后台限制。 |
...
3. 旧的 `WEKNORA_ASYNQ_CONCURRENCY` 已退休；现在必须显式设置 core/postprocess/enrichment/maintenance/shared/wiki。Thor 使用 `2/1/1/1/0/2`，其中 `shared=0` 表示禁用弹性借用，避免后台增强绕过 `enrichment=1` 的限制。
...
| --- | ---: | --- | --- |
| core | 2 | `default`: `document:process`、`manual:process` | 文档解析、分块、向量化，优先保证新文档先可检索。 |
| postprocess | 1 | `postprocess`: `knowledge:post_process` | 文字解析完成后的轻量扇出，避免被长时间文档解析阻塞。 |
...
`WEKNORA_ASYNQ_*_CONCURRENCY` 不等同于主 QA 模型并发。Thor 的后台池为 `core=2 / postprocess=1 / enrichment=1 / maintenance=1 / shared=0`，让文字解析和 embedding 继续推进，同时只有一个 enrichment worker 可能发起 Graph/VLM/Summary/Question 长请求。
...
| --- | ---: | ---: | --- | ---: | ---: | ---: | --- |
| Thor `192.168.1.81` | 7 | 3 | core=2 / postprocess=1 / enrichment=1 / maintenance=1 / shared=0 / wiki=2 | 1 | 1 | 2 / 2 | QA/Graph/Wiki/Question 共用 9B vLLM，`VLLM_MAX_MODEL_LEN=18432` 时 vLLM 实测满长有效并发约 3.96；后台主模型并发 `WEKNORA_MODEL_MAX_CONCURRENCY=1`，`WEKNORA_CHAT_MODEL_CONTEXT_TOKENS=18432`，避免影响聊天和 RAG prompt 越界。 |
...
这样 bge-m3 vLLM 最大 8 并发，文档 embedding 的应用侧并发最多 4，给聊天检索保留约 3 个服务槽位。如果聊天检索仍出现排队，应继续降低 `CONCURRENCY_POOL_SIZE` 或后台 worker，而不是提高 bge-m3 vLLM 到机器承受不了的并发。

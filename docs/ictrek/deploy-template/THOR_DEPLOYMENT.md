...
`.env.thor` keeps `WEKNORA_REPARSE_INCOMPLETE_ON_START=true` and `WEKNORA_REPARSE_WAIT_URLS=http://qwen35-9b-vllm:22222/v1/models,http://bge-m3-vllm:22223/v1/models`. After every app container recreate, LexAI waits for those model endpoints and then automatically resubmits `failed` and `pending` knowledge rows to the batch reparse queue; `processing` and `finalizing` rows are resubmitted only when `processed_at is null`. Documents whose text parsing and vector indexing already completed are not full-reparsed just because VLM/Graph/Wiki background work is still pending. This is part of the redeploy flow: model/vLLM restarts or interrupted work should recover without manual per-document retry.
...
The deploy script recreates only managed services whose image digest or deployment config changed. Database services are not restarted. When `docreader` changes, the script waits for it to become healthy; when `app` changes, it waits for app health; when vLLM services change, it waits for `WEKNORA_REPARSE_WAIT_URLS` before running `trigger-reparse-incomplete.sh` to submit current incomplete knowledge through `POST /knowledge/batch-reparse`. This script uses the same guard as app startup: it full-reparses `failed`/`pending`, and only reparses `processing`/`finalizing` when `processed_at is null`. Use `WEKNORA_TRIGGER_REPARSE_AFTER_DEPLOY=false` only for a deliberate maintenance skip.

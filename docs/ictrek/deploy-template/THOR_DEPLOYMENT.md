...
`.env.thor` keeps `WEKNORA_REPARSE_INCOMPLETE_ON_START=true` and `WEKNORA_REPARSE_WAIT_URLS=http://qwen35-9b-vllm:22222/v1/models,http://bge-m3-vllm:22223/v1/models,http://bge-reranker-vllm:22224/v1/models`. After every app container recreate, ChatSwitch waits for those model endpoints and then automatically resubmits parseable `failed` and `pending` knowledge rows to the batch reparse queue; `processing` and `finalizing` rows are resubmitted only when `processed_at is null`. Parseable means the row has a file path, a URL source, or non-empty manual content. Documents whose text parsing and vector indexing already completed are not full-reparsed just because VLM/Graph/Wiki background work is still pending, and empty rows that cannot produce a `document:process` task are skipped. This is part of the redeploy flow: model/vLLM restarts or interrupted work should recover without manual per-document retry.
...
| Default Rerank | `lexai-thor-vllm-bge-reranker` | `http://bge-reranker-vllm:22224/v1` |
...
Default Embedding is `lexai-thor-vllm-bge-m3-embedding`, served by `bge-m3-vllm` through `http://bge-m3-vllm:22223/v1` with `interface_type=openai`. Default Rerank is `lexai-thor-vllm-bge-reranker`, served by `bge-reranker-vllm` through `http://bge-reranker-vllm:22224/v1`. Ollama `bge-m3:latest` stays in the config as a non-default backup and is not preloaded. Keep `BGE_VLLM_MAX_NUM_SEQS=16`, `BGE_RERANK_VLLM_MAX_NUM_SEQS=16`, `BATCH_EMBED_SIZE=8`, and `CONCURRENCY_POOL_SIZE=8` so document ingestion can use 8 embedding requests while interactive retrieval has spare service slots. The generic tuning rules are in [CONCURRENCY.md](CONCURRENCY.md).

Model services are intentionally separated from ChatSwitch runtime services. `docker-compose.thor.models.yml` extends the Thor compose definitions for `model-hub-backend`, `model-hub-frontend`, `model-hub-ollama`, `model-hub-bootstrap`, `qwen35-9b-vllm`, `bge-m3-vllm`, and `bge-reranker-vllm`. Start this compose first and let `model-hub-bootstrap` download/export all entries in `MODEL_HUB_PRELOAD_MODELS`; entries may use `hf://...` or `ms://...`. The default list includes `hf://BAAI/bge-m3`, `hf://BAAI/bge-reranker-v2-m3`, and `hf://QuantTrio/Qwen3.5-9B-AWQ`. The vLLM containers mount `${MODEL_HUB_DATA_DIR}` as `/modelhub` and should point at the exported `current` paths, for example `/modelhub/export/hf/BAAI/bge-reranker-v2-m3/current`. If the reranker starts before that `current/config.json` exists, vLLM treats the absolute path as a remote repo id and exits; wait for Model Hub export readiness, then recreate only `bge-reranker-vllm`.

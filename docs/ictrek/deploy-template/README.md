...
Housekeeping runs every 5 minutes in the app container. It promotes `finalizing` rows whose `pending_subtasks_count=0` to `completed`, and marks drained `summary_status=pending/processing` rows as `failed` when the knowledge row is already `completed` and has no pending subtasks left. That prevents stale UI labels like "optimizing" or "generating summary" after all runnable work has already drained.

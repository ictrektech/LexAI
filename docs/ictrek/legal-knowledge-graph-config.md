...
模型、Wiki 生成、Graph 抽取、部署和重新解析的完整流程见 `wiki-graph-model-deploy-guide.md`。本文只聚焦实体、关系和示例文本怎么填。
...
1. 打开知识库 -> 设置 -> 知识图谱。
2. 确认页面没有「知识图谱数据库未启用」提示；如果有，先检查部署中的 Neo4j。
3. 打开「启用实体关系提取」。
4. 「关系类型」填入本文的关系类型列表。
5. 「示例文本」填入本文示例文本，或替换成当前业务更贴近的法律文本。
6. 「管理实体」逐个添加实体，实体属性按本文表格填写。
7. 「管理关系」逐个添加三元组：起点实体、关系、终点实体。
8. 点击「开始提取」看模型抽取出的节点/关系是否合理。
9. 点击右下角「保存配置」。

如果只想按领域裁剪，先删实体和关系，不要新增太多标签。合同类保留合同、条款、权利义务、违约、赔偿；案例类保留案件、法院、当事人、证据、争议焦点、裁判观点、法条。

...
KEYWORD_SYNONYMS = {
    ...
    "事实要素": ["事实要素", "案件事实", "关键事实", "基本案情", "事实"],
    "关键事实": ["关键事实", "案件事实", "事实要素", "基本案情", "事实"],
    "案例事实": ["案例事实", "案件事实", "基本案情", "事实要素"],
    "法律条文或原则": ["法律条文或原则", "法律条文", "法律原则", "法条", "适用依据"],
    "法条或法律原则": ["法条或法律原则", "法条", "法律原则", "法律条文", "适用依据"],
    "裁判理由": ["裁判理由", "裁判观点", "法院认定", "法院认为", "裁判要点"],
    "法律原则": ["法律原则", "原则"],
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--timeout", type=int, default=int(os.getenv("LEGAL_KG_TIMEOUT", "360")))
...
6. 控制输出规模：节点不超过 15 个，关系不超过 18 条；不要大段摘录法条或案例原文，依据只列文档名、法条编号或案例名。
7. 答案末尾必须输出“标准关系边表”，表头固定为“源节点 | 关系 | 目标节点 | 依据”，并尽量使用用户问题中指定的图谱维度词，例如主体、行为、条件、例外、责任、救济、当事人、争议焦点、法院认定、裁判结果、适用依据、案例观点、事实要素。
8. 不要输出确定性个案胜诉结论。
"""

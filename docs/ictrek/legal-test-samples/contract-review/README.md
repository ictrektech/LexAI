...
如果刚更新过合同审查 Prompt 模板或 `contract-review` Skill，需先重新部署/重启后端。使用内置「合同审查」智能体时，确认该内置配置没有被页面保存过旧的自定义配置；使用自定义智能体时，建议重新套用「合同审查」类型预设/恢复默认模板并保存，或重新创建测试智能体。否则脚本可能仍在测试旧 `system_prompt`，导致修复未完全生效。
...
如需加快合同审查专项测试，可以用 `--workers` 并发执行用例：
...
`--workers` 默认是 `1`。tc232 上建议先用 `2`，确认后端和模型服务稳定后再尝试 `3`；过高可能导致请求超时或后端拒连。
...
P1 完整报告类用例会按固定标题和风险覆盖检查。产品提示词、智能体 Skill 或人工执行时都应要求模型使用以下标题，不要合并或改名：
...
普通 P1 报告至少应覆盖 4 项风险；长篇框架、主采购、SaaS、技术服务、租赁合作等多条款商业合同，在合同文本支持时至少应覆盖 6 项风险。

P1 风险点需要点名合同中的具体触发词、数字、比例、期限或空白字段，例如预付款/付款早于交付或验收、验收期限和视为合格、争议解决空白、交付清单缺失、单方变更价格或功能、安全事件通知时限、数据导出/删除、产权/消防/授权文件、经营资质/许可、文件优先级、供应链中断和替换供货、责任限制和间接损失排除。

P1 报告还应控制篇幅，避免生成过长导致末尾章节截断：风险清单用短表格单元格，重点条款修改只写 3-5 条核心条款，缺失条款建议和待确认事项用简短项目符号，最后必须输出 `## 免责声明` 和一句话。

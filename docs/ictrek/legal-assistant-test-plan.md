...
| 接口模式 | `knowledge` |
| 结果 | 通过，`1 / 1 = 100.00%` |
| 逐轮判定 | 第 1-3 轮 `PASS`，第 4 轮 `REVIEW` |
| 同一 session_id 复用 | 是，4 轮均复用同一会话 |
| 承接点命中 | 4/4 轮命中 |
| 新增判断命中 | 3/4 轮通过；第 4 轮命中不足，需人工复核未签书面劳动合同责任表述 |
| 引用依据命中 | 4/4 轮命中，标准引用数分别为 5、7、5、9 |
| 总耗时 | 168.06s |
| 结果目录 | `/tmp/multi-turn-followup-smoke` |

执行结果摘要：

- 脚本成功通过 `--auto-setup` 获取本地 token，并为 `MTF-001` 创建 1 个临时会话；
- 4 轮追问均复用同一个 `session_id`，验证了脚本层面对多轮会话的复用；
- 第 1-3 轮同时命中前文承接、新增事实判断和引用依据；
- 第 4 轮命中前文承接和引用依据，但新增判断关键词只命中 1/3，机器判定为 `REVIEW`；

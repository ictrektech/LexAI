...
按当前状态，已准备完成的测试样例包括 **8.1 CLI RAG Full Loop E2E**、**8.2 合同审查样例自动测试**、**8.3 法律法规问答样例自动测试**、**8.4 裁判案例问答样例自动测试**、**8.5 法律知识图谱样例自动测试** 和 **8.6 无依据拒答样例自动测试**。CLI E2E 能证明基础 RAG 链路可跑通，但不能代表全部法律专项测试样例已经完成验收。

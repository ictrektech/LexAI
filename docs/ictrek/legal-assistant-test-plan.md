...
| 模块 | 权重 | 通过情况 | 得分 | 主要问题 |
| --- | ---: | ---: | ---: | --- |
| 知识库与文档入库 | 15% | CLI RAG E2E 通过 | 15.00 | 基础 RAG 闭环已通过，法律专项仍依赖既有法律条文库和案例库质量 |
| 法律问答准确性 | 30% | 24 / 25 | 28.80 | 法规问答 `LAWQA-007` 需人工复核；案例问答和多轮追问全量通过 |
| 检索与引用 | 25% | 自动证据检查基本可用，待人工抽验 | 23.00 | 各专项均返回引用或工具证据，但引用准确率尚未逐条人工定位复核 |
| 合同审查 | 15% | 20 / 20 | 15.00 | 2026-07-27 合同审查快速问答和智能推理专项回归均为 10 / 10；仍需人工抽验法律质量和引用片段 |
...
按最新合同审查回归结果更新后，自动初测加权完成度约 **93.94%**。该数值只代表脚本初筛完成度，不替代人工法律质量复核；当前主要未达标专项为法律知识图谱。
...
未显式传 `--output-root` 时，重跑结果默认写入上一轮目录下的 `rerun-<时间戳>/` 子目录，避免覆盖原始结果。对缺少 `manifest.json` 的历史手工批次，脚本会扫描原批次和 `rerun-*` 子目录中的专项结果；同一用例按最新结果判断，已经 PASS 的用例不会继续列入重跑计划。请求超时或中断导致没有写入 `results.json` 的用例，会按专项 `test-cases.json` 标为 `MISSING` 并进入默认重跑计划。
...
合同审查专项支持在总控脚本中通过 `--contract-workers <N>` 并发执行用例。2026-07-27 合同审查回归使用 `--contract-workers 5`，快速问答和智能推理两个合同审查专项均为 10 / 10；若完整法律专项批次同时包含图谱等重型任务，建议先从 `--contract-workers 4` 或 `5` 观察后端、Redis 队列和 vLLM 稳定性。
...
专项完整重跑、只重跑指定 `REFUSAL-*` 用例，以及把补跑结果写入历史完整批次 `rerun-<时间戳>/` 子目录的命令，见 [无依据拒答测试样例](legal-test-samples/no-evidence-refusal/README.md) 的“专项重跑”部分。
...
| 项目 | 结果 |
| --- | --- |
| 测试日期 | 2026-07-17；合同审查回归为 2026-07-27 |
...
该批次用于执行 4.1 中已准备的法律专项样例。测试通过 `tmux 232` 会话中的本机开发服务执行；执行时 `hostname` 返回 `a6000`，后端为 `http://localhost:8080`，前端为 `http://localhost:5173/`。后端健康检查返回 `{"status":"ok"}`。该结果按脚本机器判定记录，仍需人工复核法律准确性和引用片段。
...
| 法律案例库 | `4ca9a808-83f5-4222-8cc4-424ae24f6656` |
| 执行认证 | `--auto-setup` |
| 结果根目录 | `/tmp/legal-assistant-full-20260717` |
| 合同审查最新回归 | `docs/ictrek/legal-test-samples/results/full-20260727-152044`，`--contract-workers 5` |

完整批次结果汇总：

| 专项 | 接口 / 智能体 | 结果 | 通过率 | 结果目录 | 结论 |
| --- | --- | ---: | ---: | --- | --- |
...
| 合同审查（快速问答） | `agent` / `90fd6ab5-ba06-4bff-8a23-878ce00837ef` | 10 / 10 | 100.00% | `docs/ictrek/legal-test-samples/results/full-20260727-152044/contract-review-agent-quick` | 2026-07-27 回归通过机器初筛 |
| 合同审查（智能推理） | `agent` / `511258fd-4f3f-419e-8053-09f652ab50a5` | 10 / 10 | 100.00% | `docs/ictrek/legal-test-samples/results/full-20260727-152044/contract-review-agent-reasoning` | 2026-07-27 回归通过机器初筛 |
...
- 法律知识图谱中 `LKG-004` 在完整批次中请求超时且未写入结果；重跑其余 6 条时 `LKG-003`、`LKG-005`、`LKG-006` 因关系命中不足进入 `REVIEW`。当前已收紧图谱输出规模、要求标准关系边表，并让总控脚本把无结果用例标为 `MISSING` 纳入重跑；
- 多轮追问 5 组全部通过，并且所有用例均复用同一 `session_id`；
- 合同审查在 2026-07-27 回归中已完成提示词、合同类型覆盖清单、P1 完整报告格式约束和判分同义词修复；快速问答和智能推理两个专项均为 10 / 10。该批次使用 `--contract-workers 5` 并发执行，总耗时约 9 分 21 秒，结果目录为 `docs/ictrek/legal-test-samples/results/full-20260727-152044`；
- 完整批次执行期间观察到 vLLM 指标中长图谱请求持续占用 1 个运行请求、0 个排队请求，说明 `LKG-004` 更像超长生成或收尾超时，而不是队列堆积。
...
| 合同审查可用率 | >= 80% | 100.00% | 通过 |
| 知识图谱冒烟通过率 | 100% | 42.86% | 未通过 |
| 多轮追问通过率 | 初测参考 >= 80% | 100.00% | 通过 |

下一步建议：

1. 优先重跑法律知识图谱 `LKG-003`、`LKG-004`、`LKG-005`、`LKG-006`，重点确认标准关系边表是否提升关系命中，并观察 `LKG-004` 是否仍有超时；
2. 对法律法规问答 `LAWQA-007` 做人工复核，确认是模型遗漏、测试期望过严，还是知识库检索片段不足；
3. 对合同审查最新 20 条 PASS 做人工抽验，重点检查法律依据引用片段、风险分级和修改建议是否具备正式业务可用性；
4. 保留无依据拒答 10 条和合同审查 20 条专项作为回归集，后续提示词、检索链路或模型并发配置改动后优先重跑，防止硬拒答和合同审查能力回退；
5. 完成图谱和 `LAWQA-007` 修复后，重新执行失败和 `REVIEW` 用例，再跑一轮完整法律专项批次。

...
### 9.5 法律知识图谱样例脚本校验与 smoke 探测

该记录用于说明本轮新增法律知识图谱样例的本地脚本校验结果。由于本轮探测 `http://localhost:8080/health` 未能连接到本地服务，未执行真实 API smoke；待本地服务恢复后，可按 [法律知识图谱测试样例](legal-test-samples/legal-knowledge-graph/README.md) 中命令补跑。

| 项目 | 结果 |
| --- | --- |
| 测试日期 | 2026-07-17 |
| 测试环境 | 本地仓库脚本校验；后端 `http://localhost:8080` 探测失败 |
| 法律条文库 | `f07af6bb-2645-428a-8db2-829708e3a2c2` |
| 法律案例库 | `4ca9a808-83f5-4222-8cc4-424ae24f6656` |
...
| API smoke | 未执行，`http://localhost:8080/health` 当前无法连接 |
| 结果目录 | 未生成；服务恢复后建议输出到 `/tmp/legal-knowledge-graph-smoke` |

本轮 dry-run 已确认 7 条样例会按用例配置选择法律条文库、法律案例库或双知识库；真实回答的核心节点/关系命中和引用依据仍需在服务可用后执行 API smoke 或完整样例测试确认。
